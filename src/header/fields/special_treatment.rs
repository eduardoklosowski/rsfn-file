...
impl From<u8> for SpecialTreatment {
    fn from(value: u8) -> Self {
        match value {
            ...
            8 => Self::CompressWithZip,
            10 => Self::ComrpessWithZipWithoutCrypt,
            n => Self::Unknown(n),
        }
    }
}
...
impl SpecialTreatment {
    pub fn value(&self) -> u8 {
        match self {
            ...
            Self::CompressWithZip => 8,
            Self::ComrpessWithZipWithoutCrypt => 10,
            Self::Unknown(n) => *n,
        }
    }
    ...
    pub fn describe_value(&self) -> String {
        match self {
            ...
            Self::CompressWithZip => "Arquivo compactado com Zip".to_string(),
            Self::ComrpessWithZipWithoutCrypt => {
                "Arquivo compactado com Zip, sem cifragem".to_string()
            }
...
mod tests {
    ...
    #[test]
    fn value_compress_with_zip() {
        let sut: SpecialTreatment = [8].into();

        assert_eq!(sut, SpecialTreatment::CompressWithZip);
        assert_eq!(sut, 8.into());
        assert_eq!(sut.value(), 8);
        assert_eq!(sut.describe_value(), "Arquivo compactado com Zip");
        assert!(sut.is_valid());
        assert_eq!(sut.to_bytes(), [8]);
        assert_eq!(sut.to_string(), "0x08 [Arquivo compactado com Zip]");
    }

    #[test]
    fn value_compress_with_zip_without_crypt() {
        let sut: SpecialTreatment = [10].into();

        assert_eq!(sut, SpecialTreatment::ComrpessWithZipWithoutCrypt);
        assert_eq!(sut, 10.into());
        assert_eq!(sut.value(), 10);
        assert_eq!(
            sut.describe_value(),
            "Arquivo compactado com Zip, sem cifragem"
        );
        assert!(sut.is_valid());
        assert_eq!(sut.to_bytes(), [10]);
        assert_eq!(
            sut.to_string(),
            "0x0a [Arquivo compactado com Zip, sem cifragem]"
        );
    }
